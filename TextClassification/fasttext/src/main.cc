...
void keyphrase(const std::vector<std::string> args) {
    int32_t k;
    int method;
    if (args.size() == 3) {
        k = 5;
    ...
    if (method == 0)
    {
        while (fasttext.extractLine(in, words, k))
        {
            for (auto p : words)
    ...
    {
        while (fasttext.textrankLine(in, words, k))
        {
            for (auto p : words)
                std::cout << p.second << "\t" << p.first << std::endl;
        }
