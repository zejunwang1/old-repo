...
namespace fasttext {
 ...
 const std::string dict_path = "/home/wangzejun/local/include/dict/good_sortutf8dict.txt";
 const std::string user_dict_path = "";

 Darts::DAT dat(dict_path, user_dict_path);

 const char* const DICT_PATH = "/home/wangzejun/local/include/dict/jieba.dict.utf8";
 const char* const HMM_PATH = "/home/wangzejun/local/include/dict/hmm_model.utf8";
 const char* const USER_DICT_PATH = "/home/wangzejun/local/include/dict/user.jieba.utf8";
 const char* const IDF_PATH = "/home/wangzejun/local/include/dict/idf.utf8";
 const char* const STOP_WORD_PATH = "/home/wangzejun/local/include/dict/stop_words.utf8";
...
void Dictionary::deletePunc(std::vector<std::string>& words) const
{
    std::unordered_set<uint16_t> punc = { 0x0b7, 0x00d7, 0x2014,
    0x2018, 0x2019, 0x201c, 0x201d, 0x2026, 0x3001, 0x3002, 0x300a,
    0x300b, 0x300e, 0x300f, 0x3010, 0x3011, 0xff01, 0xff08, 0xff09,
    0xff0c, 0xff1a, 0xff1b, 0xff1f, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26,
    0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f, 0x3a, 0x3b,
    0x3c, 0x3d, 0x3e, 0x3f, 0x40, 0x5b, 0x5c, 0x5d, 0x5e, 0x5f, 0x60,
    0x7b, 0x7c, 0x7d, 0x7e };
    for (std::vector<std::string>::iterator it = words.begin(); it != words.end(); )
    {
        std::vector<uint16_t> unic;
        limonp::Utf8ToUnicode< std::vector<uint16_t> >(*it, unic);
        if (unic.size() == 1 && punc.count(unic[0]))
            it = words.erase(it);
        else
            it++;
    }
...
bool Dictionary::isHanzi(const std::string& word) const
{
    if (word.find(" ") != std::string::npos)
        return false;
    std::vector<uint16_t> unic;
    limonp::Utf8ToUnicode< std::vector<uint16_t> >(word, unic);
    for (int i = 0; i < unic.size(); i++)
    {
        if (unic[i] < 0x4E00 || unic[i] > 0x9FA5)
            return false;
    }
    return true;
}

void Dictionary::getChineseWord(std::vector<std::string>& words) const
{
    for (std::vector<std::string>::iterator it = words.begin(); it != words.end(); )
    {
        bool flag = isHanzi(*it);
        if (flag)
            it++;
        else
            it = words.erase(it);
...
void Dictionary::readFromMultiLines(std::istream& in)
{
    ...
    {
        if (line.empty() || line.length() <= 5)   continue;
        ...
        // dartseg
        //dat.cut(line, words, true);

        // delete punc
        deletePunc(words);

        // get chinese word
        getChineseWord(words);
    ...
    if (args_->verbose > 0)
    {
        std::cerr << "\rRead " << ntokens_ / 10000 << "M words" << std::endl;
    ...
    std::minstd_rand& rng) const
{
    ...
    words.clear();
    std::getline(in, line);
    if (line.empty() || line.length() < 5 || line.find(".xml") != std::string::npos)
        return ntokens;
    // delete '\r'
    if (line.find("\r") == line.length() - 1)
        line = line.substr(0, line.length() - 1);
    ...
    //dat.cut(line, ws, true);
    jieba.Cut(line, ws, true);
    deletePunc(ws);
    getChineseWord(ws);
    for (auto token : ws)
    {
