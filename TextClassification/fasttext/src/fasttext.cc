...
cppjieba::TextRankExtractor textrank(DICT_PATH,
        HMM_PATH,
        STOP_WORD_PATH);
...
void FastText::test(std::istream& in, int32_t k, real threshold, Meter& meter)
  ...
  while (in.peek() != EOF) {
    ...
    dict_->getChLine(in, line, labels);
...
bool FastText::textrankLine(std::istream& in,
    std::vector< std::pair<real, std::string> >& words,
    int32_t k) const
{
    words.clear();
    if (in.peek() == EOF)
    ...
    textrank.Extract(sentence, keywords, k);
    std::cout << keywords.size() << std::endl;

    for (size_t i = 0; i < keywords.size(); i++)
    {
        words.push_back(std::make_pair(keywords[i].second, keywords[i].first));
    }
    return true;
}

bool FastText::extractLine(std::istream& in, 
    std::vector< std::pair<real, std::string> >& words,
    int32_t k) const
{
    words.clear();
    ...
    std::getline(in, sentence);
    std::vector< std::pair<std::string, double> > keywords;
    jieba.extractor.Extract(sentence, keywords, k);
    for (size_t i = 0; i < keywords.size(); i++)
    {
        words.push_back(std::make_pair(keywords[i].second, keywords[i].first));
    }
...
bool FastText::predictLine(
  ...
  dict_->getChLine(in, words, labels);
...
void FastText::trainThread(int32_t threadId, const TrainCallback& callback) {
  ...
  try {
    while (keepTraining(ntokens)) {
      ...
      if (args_->model == model_name::sup) {
        localTokenCount += dict_->getChLine(ifs, line, labels);
        supervised(state, lr, line, labels);
      } else if (args_->model == model_name::cbow) {
        localTokenCount += dict_->getChLine(ifs, line, state.rng);
        cbow(state, lr, line);
...
std::shared_ptr<Matrix> FastText::getInputMatrixFromFile(
  ...
  std::cout << "total " << n << " words. " << dim << " dimensions." << std::endl;

  mat = std::make_shared<DenseMatrix>(n, dim);
  for (size_t i = 0; i < n; i++) {
    if (i % 10000 == 0)
        std::cerr << "loading " << i / 10000 << "w vectors.\r" << std::flush;
    std::string word;
    in >> word;
  ...
  in.close();
  std::cout << "\ndict_ size: " << dict_->size() << std::endl;
  dict_->threshold(1, 0);
  dict_->init();
